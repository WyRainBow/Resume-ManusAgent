# 上下文恢复机制

## 概述

实现页面刷新后 AI 仍能记住之前对话内容的功能，通过 WebSocket 在前后端之间同步历史消息。

---

## 实现流程

```
┌─────────────────────────────────────────────────────────────────┐
│                      流程图                                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. 用户刷新页面                                                │
│     │                                                            │
│     ▼                                                            │
│  2. 前端从 localStorage 加载历史消息                              │
│     │                                                            │
│     ▼                                                            │
│  3. WebSocket 连接建立                                          │
│     │                                                            │
│     ├─► 前端: 发送 restore_history + 历史消息                       │
│     │                                                            │
│     ▼                                                            │
│  4. 后端接收 restore_history                                     │
│     │                                                            │
│     ├─► 恢复到 global_chat_history                               │
│     └─► 恢复到 agent.memory                                     │
│     │                                                            │
│     ▼                                                            │
│  5. Agent 现在有了之前的上下文                                   │
│     │                                                            │
│     └─► 用户继续对话，AI 能记住之前的内容                        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 修改的文件

| 文件 | 修改内容 |
|------|----------|
| `frontend/src/App.jsx` | WebSocket 连接时发送历史消息 |
| `app/web/server.py` | 接收 restore_history 并恢复到 Agent |

---

## 前端实现

### localStorage 工具函数

```javascript
const STORAGE_KEYS = {
  MESSAGES: 'openmanus_chat_messages',
  RESUME_DATA: 'openmanus_resume_data',
  SHOW_RESUME_PANEL: 'openmanus_show_resume_panel'
};

const loadMessagesFromStorage = () => {
  const stored = localStorage.getItem(STORAGE_KEYS.MESSAGES);
  if (stored) {
    const parsed = JSON.parse(stored);
    console.log(`📜 恢复 ${parsed.length} 条历史消息`);
    return parsed;
  }
  return [];
};

const saveMessagesToStorage = (messages) => {
  const toSave = messages.slice(-100); // 保存最近 100 条
  localStorage.setItem(STORAGE_KEYS.MESSAGES, JSON.stringify(toSave));
};
```

### WebSocket 连接时恢复历史

```javascript
socket.onopen = () => {
  console.log('Connected to WebSocket');
  setStatus('idle');
  setWs(socket);

  // 发送历史消息给后端用于恢复上下文
  const storedMessages = loadMessagesFromStorage();
  if (storedMessages.length > 0) {
    // 只发送用户和助手的对话消息，过滤掉工具调用等
    const conversationMessages = storedMessages.filter(msg =>
      msg.role === 'user' || (msg.role === 'agent' && msg.type === 'answer')
    );

    if (conversationMessages.length > 0) {
      console.log(`📜 发送 ${conversationMessages.length} 条历史消息到后端`);
      socket.send(JSON.stringify({
        type: 'restore_history',
        messages: conversationMessages.map(msg => ({
          role: msg.role === 'user' ? 'user' : 'assistant',
          content: msg.content || ''
        }))
      }));
    }
  }
};
```

---

## 后端实现

### WebSocket 消息处理

```python
# 处理前端发来的历史消息恢复请求
if message.get("type") == "restore_history":
    history_messages = message.get("messages", [])
    if history_messages:
        logger.info(f"📜 从前端恢复 {len(history_messages)} 条历史消息")
        for msg in history_messages:
            role = msg.get("role", "user")
            content = msg.get("content", "")
            if role == "user":
                global_chat_history.add_message(Message.user_message(content))
                agent.memory.add_message(Message.user_message(content))
            elif role == "assistant":
                global_chat_history.add_message(Message.assistant_message(content))
                agent.memory.add_message(Message.assistant_message(content))
    continue
```

---

## 效果

| 功能 | 状态 |
|------|------|
| 用户刷新页面 | ✅ |
| 前端恢复聊天历史显示 | ✅ |
| 前端发送历史消息给后端 | ✅ |
| 后端恢复到 Agent memory | ✅ |
| AI 继续对话时能记住之前的上下文 | ✅ |

---

## 测试方法

1. 启动前后端服务
2. 与 AI 进行对话
3. 刷新浏览器页面
4. 继续与 AI 对话，验证 AI 是否记得之前的内容

---

## 技术细节

### 消息过滤

前端发送时只包含有效对话消息：
- `user` 类型的消息
- `agent` 且 `type === 'answer'` 的消息

过滤掉的消息类型：
- `tool_call` - 工具调用
- `tool_result` - 工具返回结果
- `step` - 步骤信息
- `thought` - 思考过程

### 消息映射

前端到后端的角色映射：

| 前端 role | 后端 role |
|-----------|-----------|
| `user` | `user` |
| `agent` (type=answer) | `assistant` |
